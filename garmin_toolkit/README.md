...
# 5. Workout Introspection (New)
workouts = provider.get_workout_templates()
for w in workouts:
    print(f"Template: {w.workout_name} ({w.sport_type})")
...
Tools generated by the `ToolFactory` now include a `.run()` method, ensuring full compatibility with LangChain `StructuredTool` expectations and making them directly callable for internal execution. The factory now exposes 10 core tools covering activities, workouts, and biometrics.
