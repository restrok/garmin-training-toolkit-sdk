"""Tests for authentication self-healing and session refresh logic."""
...
from garmin_training_toolkit_sdk.utils import (
    _refresh_garmin_session,
    get_authenticated_client,
)


class TestAuthSelfHealing(unittest.TestCase):
    """Test suite for authentication self-healing functionality."""
    ...
    @patch(
        "builtins.open", new_callable=mock_open, read_data='{"di_token": "old_token"}'
    )
    @patch("pathlib.Path.exists")
    def test_get_authenticated_client_success_first_try(
        self,
        ...
        """Tests successful client authentication on the first attempt."""
        mock_exists.return_value = True
        mock_find.return_value = self.token_file

        # Mock client behavior
        client_instance = mock_garmin.return_value
        client_instance.get_userprofile_settings.return_value = {
    ...
    @patch("pathlib.Path.exists")
    def test_get_authenticated_client_self_healing_success(
        self,
        mock_exists: MagicMock,
        mock_file: MagicMock,
        mock_garmin: MagicMock,
        ...
        """Tests successful self-healing after an initial authentication failure."""
        mock_exists.return_value = True
        mock_find.return_value = self.token_file
        ...
        success_client.get_userprofile_settings.return_value = {
            "displayName": "Test User"
        }
    ...
    @patch("pathlib.Path.exists")
    def test_get_authenticated_client_self_healing_fails(
        self,
        mock_exists: MagicMock,
        mock_file: MagicMock,
        mock_garmin: MagicMock,
        ...
        """Tests that authentication fails if self-healing also fails."""
        mock_exists.return_value = True
        mock_find.return_value = self.token_file
    ...
    @patch("pathlib.Path.exists")
    def test_refresh_garmin_session_iteration(
        self,
        mock_exists: MagicMock,
        mock_file: MagicMock,
        mock_garmin: MagicMock,
        ...
        """Tests iteration through client IDs during session refresh."""
        mock_exists.return_value = True
        ...
        # 3rd ID in DI_CLIENT_IDS
        self.assertEqual(
            client3.client.di_client_id, "GARMIN_CONNECT_MOBILE_ANDROID_DI"
        )
        mock_save.assert_called_once()
