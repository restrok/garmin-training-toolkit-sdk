"""Tests for LLM-native workout models and payload generation."""
...
from garmin_training_toolkit_sdk.uploaders.workouts import (
    create_step_with_target,
    create_workout,
)


def test_workout_step_duration_validation() -> None:
    """Tests validation logic for WorkoutStep durations and distances."""
    ...
    # Invalid: none
    with pytest.raises(
        ValueError, match="Exactly one of 'duration_mins', 'distance_m'"
    ):
        WorkoutStep(type="run")


def test_explicit_target_models() -> None:
    """Tests conversion of explicit target models to Garmin payloads."""
    # HR Target
...
def test_distance_duration_mapping() -> None:
    """Tests mapping of distance-based steps to Garmin payload end conditions."""
...
def test_repeat_group_logic() -> None:
    """Tests generation of RepeatGroupDTO payloads."""
    steps = [
        WorkoutStep(
            type="run",
            distance_m=400,
            target=PaceTarget(min_pace_seconds=240, max_pace_seconds=250),
        ),
    ...
    assert (
        repeat_step["workoutSteps"][0]["endCondition"]["conditionTypeKey"] == "distance"
    )
    assert repeat_step["endCondition"]["conditionTypeId"] == 7


def test_legacy_support() -> None:
    """Tests backward compatibility for legacy duration and target formats."""
