"""Tests for telemetry extraction from Garmin activities."""
...
class TestTelemetryExtraction(unittest.TestCase):
    """Test suite for telemetry extraction logic."""

    def test_get_activity_telemetry_new_metrics(self) -> None:
        """Tests telemetry extraction when all new metrics are present."""
        # Mock client
        client = MagicMock()
    ...
    def test_get_activity_telemetry_missing_metrics(self) -> None:
        """Verifies robustness for older devices/activities with missing metrics."""
        client = MagicMock()
        ...
        mock_details = {
            ...
            "activityDetailMetrics": [{"metrics": [1600000000000, 140.0]}],
        }
        client.get_activity_details.return_value = mock_details
