...
def debug_swim():
    ...
    try:
        ...
        raw_list = provider.client.get_activities_by_date(
        ...
        swim_raw = next(
            (
                a
                for a in raw_list
                if "test-swim" in a.get("activityName", "").lower()
                or a.get("activityType", {}).get("typeKey") == "lap_swimming"
            ),
            None,
        )
        ...
        if swim_raw:
            ...
            swim_activity = next(
                (
                    a
                    for a in activities
                    if "test-swim" in a.name.lower() or a.type == "lap_swimming"
                ),
                None,
            )
        ...
        if swim_activity:
            ...
            if splits:
                ...
                for s in splits[:3]:  # Show first 3
                    print(
                        f"  Lap {s.index}: {s.distance_m}m | Strokes: {s.strokes} | SWOLF: {s.avg_swolf}"
                    )
            else:
